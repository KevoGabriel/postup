...
export const Sidebar = ({ storageKey = "pu-sidebar-state" }: SidebarProps) => {
  const [expanded, setExpanded] = useLocalStorage<Record<string, any>>(
    storageKey,
    {}
  ); // Usa o hook useLocalStorage para gerenciar o estado de expansão dos itens do sidebar
  ...
  const defaultAccordionValue: string[] = Object.keys(expanded).reduce(
    (acc: string[], key: string) => {
      // Cria um array com as chaves dos itens que estão expandidos
      if (expanded[key]) {
        acc.push(key);
      }
      return acc;
